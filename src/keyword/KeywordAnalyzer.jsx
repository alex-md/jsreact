...
const TextInput = ({ onTextChange, initialText }) => {
    return (
        <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6 hover:shadow-xl transition-shadow duration-300">
            <div className="mb-6">
                <h2 className="font-bold text-gray-900">Input Text</h2>
                <p className="text-sm text-gray-600 mt-1">Enter or upload text to analyze.</p>
            </div>
            <textarea
                value={initialText}
                onChange={(e) => onTextChange(e.target.value)}
                placeholder="Paste your content here or upload a file..."
                className="text-sm w-full h-48 px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 focus:bg-white transition-all duration-200 placeholder-gray-400 text-gray-900 resize-none"
            />
        </div>
    );
};
...
const FileUpload = ({ handleFileLoad }) => {
    ...
    return (
        <div className="flex flex-col items-center justify-center p-6 bg-gray-50 border-2 border-dashed border-gray-200 rounded-lg hover:border-primary-300 transition-colors duration-200">
            ...
            <p className="mt-2 text-sm text-gray-500">Supported formats: TXT, MD</p>
        </div>
    );
};
    ...
    return (
        <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6 hover:shadow-xl transition-shadow duration-300">
            <div className="mb-6">
                ...
                <p className="text-sm text-gray-600 mt-1">Set keywords, matching, and analysis window.</p>
            </div>
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                        <label className="text-sm block text-sm font-medium text-gray-700">Matching Strategy</label>
                        <select
                            value={matchingStrategy}
                            onChange={(e) => onMatchingStrategyChange(e.target.value)}
                            className="text-sm w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 focus:bg-white transition-all duration-200 text-gray-900 cursor-pointer hover:bg-gray-100"
                        >
                    ...
                    <div className="space-y-2">
                        <label className="text-sm block text-sm font-medium text-gray-700 flex items-center justify-between">
                            Content Window Size
                            <span className="inline-flex items-center gap-1 text-sm text-primary-600 bg-primary-50 px-2 py-1 rounded-md">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            />
                            <span className="text-sm text-gray-600 min-w-[4rem] text-right">{windowSize} words</span>
                        </div>
                        <p className="text-sm text-gray-500 mt-1">Adjust window (10-{effectiveMaxWindowSize} words). Applied on analysis.</p>
                    </div>
                </div>
                {keywords.length > 0 && (
                        ...
                        {keywords.map((keyword, index) => (
                            <div
                                ...
                                className={`group flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium border transition-colors duration-200
                                            ${utils.getKeywordColor(index)} bg-opacity-20 border-current text-current`}
                            >
                                {keyword}
                                <button
        ...
        <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl shadow-lg border border-purple-100 p-6 hover:shadow-xl transition-all duration-300">
            <div className="flex flex-wrap items-center justify-between mb-6 gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <h2 className=" font-bold text-gray-900">Full Text Analysis</h2>
                </div>
                <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-gray-600">
                    <span>Words: <span className="font-medium text-gray-800">{wordCountForDisplay}</span></span>

                    <span>Overall Density: <span className="font-medium text-gray-800">{overallDensity.toFixed(2)}%</span></span>
                    <div className="flex items-center gap-1 px-2 py-1 bg-purple-100 rounded-md">
                        <span className="text-sm text-purple-600">🎙️ {speakingTime}</span>
                    </div>
                </div>
            </div>
            {keywords.length > 0 && (
                <div className="mb-4 flex flex-wrap gap-2">
                    {keywords.map((keyword, index) => (
                        <div
                            ...
                            className={`inline-flex items-center px-2 py-1 rounded-md text-sm font-medium ${utils.getKeywordColor(index)} bg-opacity-60`}
                        >
                            {keyword}
                        </div>
                    ))}
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-lg p-4 sm:p-6 border border-purple-100">
...
const ResultsDisplay = ({ text, keywords, matchingStrategy, windowSize, utils }) => {
    ...
    return (
        <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-xl shadow-lg border border-emerald-100 p-6 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-0.5">
            <div className="flex flex-wrap items-center justify-between mb-6 gap-3">
                <div className="flex items-center gap-3">
                    <span className="text-2xl">📊</span>
                    <h2 className=" font-bold text-gray-900">Keyword Performance</h2>
                </div>
                <div className="flex items-center gap-1 px-2 py-1 bg-emerald-100 rounded-md">
                    <span className="text-sm text-emerald-600">🎙️ {speakingTimeForTotal} (full text)</span>
                </div>
            </div>
            <div className="space-y-6">
                ...
                <div className="bg-white/80 backdrop-blur-sm rounded-lg border border-emerald-100 overflow-hidden">
                    <div className="grid grid-cols-4 gap-4 p-3 sm:p-4 border-b border-emerald-100 bg-emerald-50/50 text-sm ">
                        <div className="font-medium text-gray-700 col-span-2">Keyword</div>
                        <div className="font-medium text-gray-700 text-right">In Window</div>
                        <div className="font-medium text-gray-700 text-right">Overall</div>
                    </div>
                    <div className="divide-y divide-emerald-100 text-sm"> {/* Removed max-h and overflow here if it was present */}
                        {resultsForTable.map((result, index) => (
                            <div key={`${result.keyword}-${index}-results`} className="grid grid-cols-4 gap-4 p-3 sm:p-4 hover:bg-emerald-50/30 transition-colors">
                                <div className={`font-medium col-span-2 ${utils.getKeywordColor(keywords.indexOf(result.keyword))} bg-opacity-20 px-2 py-1 rounded-md truncate`}>{result.keyword}</div>
                                <div className="text-gray-800 text-right">{result.countInCluster}</div>
                                <div className="text-gray-800 text-right">
                                    {result.overallCount} <span className="text-sm text-gray-500">({result.overallDensity.toFixed(1)}%)</span>
                                </div>
                            </div>
                        ))}
                        {resultsForTable.length === 0 && <p className="p-4 text-center text-gray-500">No keywords found in the best window.</p>}
                    </div>
                </div>
            </div>
        </div>
...
const StatCard = ({ title, value, unit = '', color = 'gray' }) => (
    <div className={`bg-white/70 backdrop-blur-sm rounded-lg p-3 sm:p-4 border border-${color}-100`}>
        <p className={`text-sm  font-medium text-${color}-600 mb-1 truncate`}>{title}</p>
        <p className=" sm:text-2xl font-bold text-gray-900">
...
const HighestDensityClusterDisplay = ({ text, keywords, windowSize, matchingStrategy, utils }) => {
                ...
                {keywords.map((keyword, originalKeywordIndex) => {
                    ...
                    return (
                        <div key={keyword + originalKeywordIndex} className="relative group flex-1 flex flex-col justify-end items-center" title={`${keyword}: ${count}`}>
                            <div
                                className={`w-3/4 ${utils.getKeywordColor(originalKeywordIndex)} bg-opacity-60 rounded-t transition-all duration-300 ease-out group-hover:bg-opacity-100`}
                                style={{ height: `${Math.max(2, heightPercentage)}%` }}
                            />
                            <p className="text-sm text-gray-500 mt-1 truncate w-full text-center group-hover:font-semibold">{keyword.length > 7 ? keyword.substring(0, 5) + '..' : keyword}</p>
    ...
    return (
        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl shadow-lg border border-indigo-100 p-6 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-0.5">
            <div className="flex items-center gap-3 mb-6">
                <span className="text-2xl">🌟</span>
                <h2 className=" font-bold text-gray-900">Highest Density Cluster</h2>
            </div>
            {cluster ? (
                <div className="space-y-6">
                    <div>
                        <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-3">Top Combined Density Section</h3>
                        <div className="bg-white/80 backdrop-blur-sm rounded-lg p-4 border border-indigo-100">
                            <div className="flex flex-wrap justify-between items-start mb-3 gap-2">
                                ...
                                <div className="flex items-center gap-1 px-2 py-1 bg-indigo-100 rounded-md">
                                    <span className="text-sm text-indigo-600">🎙️ {utils.calculateSpeakingTime(cluster.wordCount)}</span>
                                </div>
                            </div>
                            {/* MODIFIED: Removed max-h-[200px] and overflow-y-auto */}
                            <div
                                className="text-gray-800 font-medium p-3 bg-white rounded-md border border-indigo-100 prose prose-sm max-w-none"
                            ...
                            {keywords.length > 1 && Object.keys(cluster.keywordCounts).length > 0 && (
                                <div className="mt-4 p-3 bg-white rounded-md border border-indigo-100">
                                    <p className="text-sm font-medium text-indigo-600 mb-2">Keyword Distribution in this Cluster</p>
                    ...
                    {cluster.individualClusters && cluster.individualClusters.length > 0 && (
                        <div>
                            <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-3 mt-6">Best Sections per Individual Keyword</h3>
                            <div className={`grid gap-4 ${cluster.individualClusters.length === 1 ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2'
                                ...
                                {cluster.individualClusters.map((individualCluster, index) => (
                                    <div key={`${individualCluster.keyword}-${index}-individual`} className="bg-white/80 backdrop-blur-sm rounded-lg p-4 border border-indigo-100">
                                        <div className="flex flex-wrap items-center justify-between mb-2 gap-2">
                                            <div className="flex items-center gap-2">
                                                <div className={`${utils.getKeywordColor(keywords.indexOf(individualCluster.keyword))} bg-opacity-60 px-2 py-0.5 rounded text-sm font-medium`}>{individualCluster.keyword}</div>
                                                <span className="text-sm  text-gray-600">{individualCluster.count} matches</span>
                                            </div>
                                            <div className="flex items-center gap-2 text-sm ">
                                                <span className="font-medium text-indigo-600">Density: {individualCluster.density.toFixed(1)}%</span>
                                                <div className="flex items-center gap-1 px-2 py-1 bg-indigo-100 rounded-md">
                                                    <span className="text-sm text-indigo-600">🎙️ {utils.calculateSpeakingTime(individualCluster.wordCount)}</span>
                                                </div>
                                            </div>
                                        </div>
                                        {/* MODIFIED: Removed max-h-[150px] and overflow-y-auto */}
                                        <div
                                            className="text-sm text-gray-800 p-3 bg-white rounded-md border border-indigo-50 prose prose-sm max-w-none"
                ...
                <div className="bg-white/80 backdrop-blur-sm rounded-lg p-6 border border-indigo-100">
                    <p className="text-gray-600 text-center">No significant keyword clusters found with the current settings. Try adjusting window size or keywords.</p>
                </div>
            )}
        </div>
    );
