// Export as global component and ES module
const ResultsDisplay = ({ text, keywords, matchingStrategy, windowSize, utils, isLoading }) => {
    ...
    if (isLoading) {
        return h('div', {
            className: 'text-gray-500 text-center py-16 animate-pulse'
        },
            h('p', { className: 'text-lg font-medium' }, 'Analyzing text...')
        );
    }
    ...
    return h('div', {
        ...
        h('div', {
            ...
            h('h2', { className: 'text-2xl font-bold text-gray-900 mb-6' }, 'Keyword Analysis Overview'),
            h('div', {
                ...
                overallStats.map(({ keyword, density, count }, index) =>
                    h('div', {
                        ...
                        h('div', { className: 'space-y-1' },
                            ...
                            h('div', { className: 'w-full bg-gray-100 rounded-full h-2 mt-2 overflow-hidden' },
                                h('div', {
        ...
        // Individual Clusters Section
        h('div', { className: 'grid grid-cols-1 lg:grid-cols-1 gap-6' },
            individualClusters.map(({ keyword, clusters }, index) =>
                clusters.length > 0 && h('div', {
                    ...
                    style: { animation: `slideIn 0.5s ease-out ${(index + overallStats.length) * 0.1}s both` }
                },
                    h('h3', { className: 'text-xl font-bold text-gray-900 mb-4' },
                        `Peak Density: "${keyword}"`
                    ),
                    h('div', { className: 'bg-gray-50 rounded-lg p-4 border border-gray-100' },
                        h('div', { className: 'flex flex-wrap gap-3 mb-3' },
                            h('div', { className: 'px-3 py-1 rounded-full bg-primary-100 text-primary-700 text-sm font-medium' },
                        ...
                        h('div', { className: 'text-sm text-gray-600 mb-3 font-medium' },
        ...
        h('div', {
            ...
            style: { animation: 'slideIn 0.5s ease-out 0.8s both' }
        },
            h('h3', { className: 'text-md font-bold text-gray-900 mb-4' }, 'Full Text Analysis'),
            h('div', {
...
console.log('ResultsDisplay.js loaded successfully');
