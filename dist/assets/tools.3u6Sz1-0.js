import{c as a,a as i}from"./footer.CxdUKtjX.js";import{c as n}from"./head.Ciumq24I.js";n("Web Tools Hub","A collection of free developer tools to simplify your workflow");a();async function c(){const t=document.getElementById("tool-cards-container");if(!t){console.error("Tool cards container not found!");return}try{const r=await fetch("/data/tools.json");if(!r.ok)throw new Error(`HTTP error! status: ${r.status}`);(await r.json()).forEach(e=>{const s=document.createElement("article");s.className="tool-card group relative overflow-hidden rounded-xl border border-gray-200 bg-white p-6 transition-all hover:shadow-lg hover:-translate-y-1";const o=e.new?`
